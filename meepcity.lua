...
local guiname = "   " .. MarketplaceService:GetProductInfo(game.PlaceId).Name .. "      Script Echo Por THIAGO"

if Players.LocalPlayer.UserId == 570260666 then -- little something for me
	guiname = "   " .. MarketplaceService:GetProductInfo(game.PlaceId).Name .. "      Dueño GUI"
end
...
local Shop = Window:AddTab("Tienda")
local Fishing = Window:AddTab("Pesca")
local Servers = Window:AddTab("Servidor")
local Reckless = Window:AddTab("Troll")
local ToysW = Window:AddTab("Juguetes")
...
local toyD = ToysW:AddDropdown("Seleccione el juguete",function(toyname)
...
ToysW:AddButton("Carga del juguete",function()
	if selectedtoy then
...
Avatar:AddLabel("Cargar Avatar")

Avatar:AddTextBox("Cargar avatar desde el ID de usuario",function(userid)
...
Avatar:AddTextBox("Cargar avatar desde el nombre de usuario",function(username)
...
clipbuttons:AddButton("Copiar el avatar actual",function()
...
clipbuttons:AddButton("Cargar Avatar",function()
	if avatarclipboard then
		ConnectionEvent:FireServer(315,avatarclipboard,true)
	end
end)

clipbuttons:AddButton("Cargar el Combo de Pelo",function()
	if avatarclipboard then
...
clipbuttons:AddButton("Guardar Avatar",function()
	if avatarclipboard then
...
Avatar:AddLabel("Diversión")

Avatar:AddButton("Cabezón",function()
	local data = Connection:InvokeServer(Constants.AE_REQUEST_AE_DATA)
	local wearing = data.PlayerCurrentTemporaryOutfit or data.PlayerCurrentlyWearing
...
Avatar:AddButton("Cabeza pequeña",function()
	local data = Connection:InvokeServer(Constants.AE_REQUEST_AE_DATA)
	local wearing = data.PlayerCurrentTemporaryOutfit or data.PlayerCurrentlyWearing
	wearing.HeadScale = 0
	ConnectionEvent:FireServer(315,wearing,true)
...
Avatar:AddButton("Altura Enorme",function()
	local data = Connection:InvokeServer(Constants.AE_REQUEST_AE_DATA)
	local wearing = data.PlayerCurrentTemporaryOutfit or data.PlayerCurrentlyWearing
	wearing.HeadScale = 99999
...
Avatar:AddButton("Altura Pequeña",function()
	local data = Connection:InvokeServer(Constants.AE_REQUEST_AE_DATA)
	local wearing = data.PlayerCurrentTemporaryOutfit or data.PlayerCurrentlyWearing
	wearing.HeadScale = 0
...
Avatar:AddButton("Desnudo",function()
	local data = Connection:InvokeServer(Constants.AE_REQUEST_AE_DATA)
	local wearing = data.PlayerCurrentTemporaryOutfit or data.PlayerCurrentlyWearing
	wearing.Shirt = "0"
	wearing.Pants = "0"
...
Servers:AddButton("Unirse al servidor más lleno",function()
	local server = getservers()[1]
	joinserver(server.InstanceId)
end)

Servers:AddButton("Unirse al servidor menos lleno",function()
	local servers = getservers()
	local server = servers[#servers]
	joinserver(server.InstanceId)
end)

Servers:AddButton("Únase a un servidor aleatorio",function()
	local servers = getservers()
...
Extra:AddSwitch("Loco",function(bool)
	if bool then
		if crazyloop then
...
Avatar:AddSwitch("Arco iris",function(bool)
	if bool then
		if rainbowloop then
...
Avatar:AddSwitch("Piel de camaleón",function(bool)
	if bool then
		if chameleon then
...
Fishing:AddSwitch("Tamaño de cubo ilimitado para el cliente",function(b)
	if b then
...
AvatarEditor:AddSwitch("Sin trajes máximo",function(b)
	if b then
...
AvatarEditor:AddTextBox("Equipar AssetId",function(assetid)
...
Extra:AddSwitch("Letras del susurro ilimitada",function(b)
	if b then
...
Reckless:AddSwitch("Notificación de teletransporte de spam para Todos",function(bool)
	if bool then
		if notifloop then
...
Extra:AddButton("Generar monedas",function()
...
Extra:AddButton("Solicitud de amistad a Todos",function()
	for _,player in pairs(Players:GetPlayers()) do
		coroutine.wrap(Connection.InvokeServer)(Connection,158,3,player.UserId)
	end
end)

Extra:AddTextBox("Susurrar a todos los amigos (puede ser lento)",function(message)
...
local Settings = {
	InviteCode = "https://discord.gg/NFEbZ6sZcp" --add your invite code here (without the "https://discord.gg/" part)
}
...
if not RequestFunction then
	return print("Su ejecutor no admite solicitudes http.")
end
